...
prayTimes = PrayTimes()
# coords = (53.254832314, 6.735997056)
coords = (43.24, 76.915)

print('Prayer Times for today in Groningen, Netherlands\n' + ('=' * 41))
times = prayTimes.getTimes(date.today(), coords, 5)
cols = [
    "asr", "dhuhr", "fajr", "imsak", "isha", "maghrib", "midnight", "sunrise", "sunset"
]
for i in cols:
...
df = pd.DataFrame(
    columns=["prayer", "date", "time"],
)
for year in tqdm(range(2000, 2050)):
    for month in range(1, 13):
        for day in range(1, 32):
            try:
                times = prayTimes.getTimes((year, month, day), coords, 2)
                times = pd.DataFrame.from_dict(times.items())
                times.columns = ["prayer", "time"]
                times["date"] = date(year, month, day)
...
# %%
a = df.copy()
a["date"] = df.date.astype("datetime64[s]").dt.month.astype(str).str.pad(2, fillchar="0") + \
    "-" + \
    df.date.astype("datetime64[s]").dt.day.astype(str).str.pad(2, fillchar="0")

a = a.pivot_table(
    index="date", columns="prayer", values="time", aggfunc=["mean", "std"]
).sort_index()
...
for i in cols:
    ax.plot(a.index, a["mean"][i], label=i)
